...
variation on a biological system. The goal of systems genetics to partition this variation 
into several classes: Genetic, Environmental and Error variation, and explain how the observed 
traits is the results of these three major factors. To partition variation we need to be able 
to construct maps of DNA inheritance, deal with data from multiple environments and keep an open 
mind for any confounding factors (errors) introduced. In this thesis we'll first introduce the 
concept and mehodologies underlying systems genetics. After which we'll go into the different 
contributions made to the field of systems genetics which are summarized by chapter in this thesis.
...
In the final chapter we describe current work on using differences in correlation to generate 
