...
This chapter highlights the conclusions of this thesis and there is a short 
discussion in which we look forward to the future of high throughput computing in 
systems genetics. Highlighting some new innovations, and outline possible issues 
which might become more relevant to systems genetics research in the coming years.

\subsection*{High throughput analysis}
\addcontentsline{toc}{subsection}{High throughput analysis}
...
\addcontentsline{toc}{subsubsection}{Cluster computing}
...
and the scheduling system put constraints on the usage of cluster. Computing clusters can be 
...
\begin{itemize}
  ...
  \item Dedicated computer clusters, such as PBS or national GRIDs, where usually a homogenous system of linux machines is used for computational tasks.
...
\addcontentsline{toc}{subsubsection}{Cloud computing}
...
\addcontentsline{toc}{subsection}{Code sharing and versioning}
...
\addcontentsline{toc}{subsection}{Pheno2Geno}
Generating biomarkers from micro arrays, tilling arrays and RNAseq data is possible using 
mixture models. Pheno2Geno provided highly optimized routines for generating genetics maps 
de-novo or to saturate existsing genetic maps. Pheno2Geno has output structures compatible 
with R/qtl, which provides further facilities for QTL mapping in inbred populations.
...
\addcontentsline{toc}{subsection}{xQTL workbench}
...
\addcontentsline{toc}{subsubsection}{wormQTL and wormQTL - Human Disease}
...
\subsection*{R/qtlHD}
\addcontentsline{toc}{subsection}{R/qtlHD}
...
\item \emph{Build in Unit Tests} - Unit testing is build into the D programming language, allowing programmers to use the 
...
\item \emph{Compile Time Function Execution} - Compile time function execution (CTFE) is a feature that allows to
build look up tables at compile time. This has the opportunity to greatly reduce the run time of 
...
Additionally multiple QTL mapping (MQM) from R/qtl will also be converted into the D programming language.
...
load when compared to single marker QTL mapping \cite{Arends:2010}. The R/qtl development team is committed to 1) add the Multiple QTL 
...
is available as preview for download. To download a development version, go to Github: 
http://github.com/qtlHD/qtlHD

Further considerations should include integration of mapping analysis tools with genomic 
annotation and sequence data. Another goal should be the generalization of analysis 
frameworks to accommodate not only mouse and human data but also data from model species 
with different genetic pedigree structures, such as pets (e.g. cats, fish, dogs), farm 
animals (e.g. horses, cattle, pigs) and plants (e.g. arabidopsis, brassica, maize).

\newpage

\section*{Dutch Summary / Nederlandse Samenvatting}
\addcontentsline{toc}{section}{Dutch Summary / Nederlandse Samenvatting}
...
Hoofdstuk 2 (Pheno2Geno) gaat over het construeren van genetische kaarten vanuit grootschalige 
...
Hoofdstuk 3 omschrijft de implementatie van het multiple QTL mapping algorithme ontwikkeld door 
...
In hoofdstuk 4 presenteren we onze ideeën voor een generiek opslag en rekenplatform 
...
netwerken te genereren en celtype specifieke QTL effecten op te sporen wordt beschreven in hoofdstuk 5. 
Correlated Traits Locus analyse (of CTL mapping) laat onderzoekers toe om genetische loci te vinden die 
geassocieerd zijn met correlatie verschillen tussen segregerende phenotypes. Een variant op deze 
methode is waardevol gebleken om cel type-specifieke eQTL effecten te ontdekken. Deze effecten kunnen 
worden gebruikt om celmengsels te ontwarren zals whole blood gen-expressie data.

Ik hoop dat je geniet van het lezen van dit proefschrift, net zo veel als ik genoot van het maken 
ervan gedurende de afgelopen vier jaar, \\\\
